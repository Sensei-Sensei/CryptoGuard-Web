...
const App: React.FC = () => {
  ...
  const handleAction = async (action: 'encrypt' | 'decrypt') => {
    if (!message || !password) {
      setError('Message and Secret Key are required.');
      return;
    }
    setIsLoading(true);
    setError('');
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: 'CryptoGuard Result',
        text: result,
      }).catch((err) => setError('Sharing failed or was cancelled.'));
    } else {
        handleCopy();
        setError('Share API not supported. Copied to clipboard instead.');
    }
  };
  ...
  const handleGeneratePassword = () => {
    ...
    setToast('New secret key generated');
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-4">
      <div className="w-full max-w-2xl mx-auto bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-2xl p-6 md:p-8 space-y-6 border border-gray-700">
        <header className="text-center">
          ...
          <p className="text-gray-400 mt-2">Securely protect and reveal your messages.</p>
        </header>
        ...
        <div className="space-y-4">
          <textarea
            ...
            aria-label="Message to encrypt or decrypt"
          />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            {isLoading ? 'Processing...' : 'Protect Message'}
          </button>
          <button
          ...
          >
            {isLoading ? 'Processing...' : 'Reveal Message'}
          </button>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <textarea
                ...
                aria-label="Encryption or Decryption Result"
              />
              <div className="absolute top-2 right-2 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <IconButton onClick={handleCopy} label="Copy Result" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>} />
                <IconButton onClick={handleShare} label="Share Result" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12s-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.368a3 3 0 105.367 2.684 3 3 0 00-5.367-2.684z" /></svg>} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-center space-x-2 border-t border-gray-700 pt-4">
           <IconButton onClick={handlePaste} label="Paste Message" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>} />
           <IconButton onClick={handleGeneratePassword} label="Generate Secret Key" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>} />
           <IconButton onClick={handleClear} label="Clear All Fields" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>} />
        </div>
      </div>
